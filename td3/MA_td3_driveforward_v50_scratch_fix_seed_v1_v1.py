...
Almost exact copy of td3_driveforward_v50.py
But here seed will be fixed.
Atempt to fix randomness
Also this script is used for eval. I just tune the hypermeters according to the trained zip file requirements.

All are same as of original except:
if abs(steering) > 0.3:
            reward -= 0.4    #changed from 0.5

Train this for 200k steps
"""
...
class SimpleTD3Wrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        norm_velocity, steering, acceleration, lat_norm, front_distance_norm = self._process_obs(info)
    ...
    def _process_obs(self, info):
        ...
        return np.array([velocity_norm, steering, acceleration, lat_norm, front_dist_norm], dtype=np.float32)
...
if False:
    ...
    # 6. Train
    # =====================================================
    model.learn(total_timesteps=200_000, log_interval=10, callback=EpisodeDebugCallback())
    print("Full timesteps :", model.num_timesteps)  #added by me
    print("TRAIN FINISHED, now saving model...")   #added by me
    model.save("MA_td3_driveforward_v50_scratch_fix_seed_v1_v1.zip")
...
model = TD3.load("MA_td3_driveforward_v50_scratch_fix_seed_v1_v1.zip",env_eval)
